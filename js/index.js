...
function getAjaxHtml() {
    ...
    xhr.onreadystatechange = () => {
        if ( xhr.readyState === 4 && xhr.status === 200) {
            document.querySelector('.html-conteiner')
...
document.querySelector('.get-fetch-html')
    .addEventListener('click', getFetchHtml);

function getFetchHtml() {
    fetch('client-data.html').then(response => response.text())
        .then(html => document.querySelector('.html-conteiner')
                .innerHTML = html)
}
...
function getAjaxJson() {
    ...
    xhr.onreadystatechange = () => {
        if ( xhr.readyState === 4 && xhr.status === 200 ) {
            ...
            document.querySelector('.client-account')
...
document.querySelector('.get-fetch-json')
    .addEventListener('click', getFetchJson);

function getFetchJson() {
    fetch('client-data.json').then(response => response.json())
        .then(clientData => {
            document.querySelector('.client-name')
                .innerText = clientData.name;
            document.querySelector('.client-account')
